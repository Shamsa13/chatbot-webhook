...
async function searchKnowledgeBase(userText) {
  try {
    ...
    const { data: chunks, error } = await supabase.rpc('match_kb_chunks', {
      query_embedding: queryEmbedding, match_threshold: 0.3, match_count: 3 
    });
...
async function updateMemorySummary({ oldSummary, userText, assistantText }) {
  ...
  const prompt = [
    "You are a strict memory archiver for an AI assistant. NEVER delete or condense existing memory lines.",
    "Extract NEW specific facts and APPEND them to the bottom.",
    "STRICT FORMATTING RULE: [CHANNEL] [YYYY-MM-DD] [TAG] Fact.",
    "Replace [CHANNEL] with [SMS] or [VOICE].",
    `Replace [YYYY-MM-DD] with [${today}].`,
    "Replace [TAG] with ONE of these: [NAME], [COMPANY], [FACT], [SUBJECT], [PREFERENCE], [GOAL], [ACTION].",
    "",
    ...
    "",
    "Return the ENTIRE memory list (existing lines + new lines appended to the bottom)."
  ].join("\n");
...
// FIXED: Mathematical/Chronological Intent Logic
...
async function checkAndSendVCard(userId, rawPhone) {
  try {
    ...
    if (!user?.vcard_sent && process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && process.env.TWILIO_PHONE_NUMBER) {
...
app.post("/twilio/sms", async (req, res) => {
  ...
  try {
    ...
    await supabase.from("messages").insert({ conversation_id: conversationId, channel: "sms", direction: "user", text: body, provider: "twilio" });
    ...
    const replyText = await callModel({ systemPrompt: cfg.systemPrompt, profileContext, ragContext, memorySummary, history: formattedHistoryForOpenAI, userText: `(SMS) ${body}` });
    const cleanReplyText = replyText.replace(/^[\(\[].*?[\)\]]\s*/, '').trim();

    await supabase.from("messages").insert({ conversation_id: conversationId, channel: "sms", direction: "agent", text: cleanReplyText, provider: "openai" });
...
app.post("/elevenlabs/twilio-personalize", async (req, res) => {
  try {
    ...
    return res.status(200).json({ dynamic_variables: { memory_summary: memorySummary || "No previous memory.", caller_phone: phone, channel: "call", recent_history: formatRecentHistoryForCall(history) || "No recent history.", first_greeting: greeting, user_name: userRecord?.full_name || "Unknown", user_email: userRecord?.email || "Unknown" } });
  } catch (err) {
...
app.post("/elevenlabs/post-call", async (req, res) => {
  try {
    ...
    const phone = normalizeFrom(String(data.metadata?.caller_id || data.user_id || body.caller_id || body.from || "").trim());
    ...
    if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && process.env.TWILIO_PHONE_NUMBER) {
      ...
      setTimeout(async () => {
        try {
          ...
          await twilioClient.messages.create({ body: textMessage, from: process.env.TWILIO_PHONE_NUMBER, to: phone.startsWith("+") ? phone : "+" + phone });
